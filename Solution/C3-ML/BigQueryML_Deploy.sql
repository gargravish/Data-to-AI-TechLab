...
CREATE OR REPLACE TABLE
  ...
  FROM
    ground_truth AS gt
  LEFT JOIN
    tx.customer_spending_features AS customer
  ON
    gt.customer = customer.customer_id
    AND gt.timestamp = customer.feature_ts
  LEFT JOIN
    tx.terminal_risk_features AS terminal
  ON
    gt.terminal = terminal.terminal_id
    AND gt.timestamp = terminal.feature_ts
  WHERE
    gt.timestamp BETWEEN TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 15 DAY) AND TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 5 DAY)
...
CREATE OR REPLACE TABLE
  ...
  ON
    ...
    AND gt.timestamp = terminal.feature_ts
  WHERE
    gt.timestamp BETWEEN TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 5 DAY) AND CURRENT_TIMESTAMP()
  AND tx_fraud IS NOT NULL);
...
--use xgboost model
--takes 18 min 49 sec with class imbalance, 5 min 19 sec without
CREATE OR REPLACE MODEL
  tx.fraud_detection_xgboost OPTIONS( model_type='BOOSTED_TREE_CLASSIFIER',
    ...
    learn_rate=0.1,
    early_stop=TRUE,
    class_weights = [STRUCT('1',.02), STRUCT('0',1)],
    l1_reg=0.1,
    l2_reg=0.1,
    subsample=0.8,
